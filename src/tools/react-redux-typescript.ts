...
export function actionCreator<T extends string, S, M, P>(type: T, payloadSelector: PayloadSelector<S, M, P>): PayloadActionCreator<T, S, M, P>;
export function actionCreator<T extends string>(type: T): EmptyActionCreator<T>;
export function actionCreator<T extends string, S, M, P>(type: T, payloadSelector?: PayloadSelector<S, M, P>,): PayloadActionCreator<T, S, M, P> | EmptyActionCreator<T> {
  if (payloadSelector == null) {
    const actionCreator: EmptyActionCreator<T> = () => ({ type });
    actionCreator.type = type;
    return actionCreator;
  }
  else {
    const actionCreator: PayloadActionCreator<T, S, M, P> = (state: S, meta?: M) => ({ type, payload: payloadSelector(state, meta) });
    actionCreator.type = type;
    return actionCreator;
  }
}
